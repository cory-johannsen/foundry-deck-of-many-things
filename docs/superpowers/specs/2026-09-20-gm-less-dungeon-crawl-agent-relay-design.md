...
**Authorization of requests (binding, not a planning-level detail).** The GM-side handler must never blindly execute an incoming request just because it arrived over the socket — a forged or scripted request from any connected client would otherwise be executed with full GM privileges. Before dispatching to `DUNGEON_ACTIONS`, the handler re-derives the run from the request's own `sceneId` (`getRunState`) and requires `run.hostUserId === msg.requestingUserId` for every action except `startRun`, which instead requires no *other* host already owns an active run on that scene (the same collision check `openDungeon()` uses). This is the request-routing model's equivalent of the archived design's `canActOnDungeon` check — moved from "may I write locally" to "should I honor this ask" — and it is what stops a read-only spectator (whose own client already hides the buttons, per the broadcast mechanism) from mutating state even if they bypass the UI and emit a request directly.
