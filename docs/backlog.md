...
**Batch 4 (all 41 level 0 creatures) — level 0 tier now fully complete.** `node tools/generate-token-art.mjs` run against all 41 level 0 `MONSTER_ART` entries. Initial pass: only 13 of 41 came back clean — the worst first-pass yield of any batch so far.

**A new cross-cutting failure class found and fixed at the source.** Roughly 25 of 41 first-pass images came back on a flat or gradient grey (occasionally white) studio backdrop instead of black. Neither `STYLE`'s "black background" wording nor `NEGATIVE`'s `colored background` cluster caught this — grey/white aren't "colored," and nothing was telling the model not to fall back to SDXL's own default neutral-grey studio-portrait backdrop. Fixed for all future batches: `STYLE` gained an explicit "not grey, not white" clause, and `NEGATIVE` gained a dedicated grey/white-backdrop cluster plus terms for the batch's other recurring miss (open-landscape/splashing-water scenery).

**A real mistake made and caught mid-batch:** a regex-based script meant to bake an explicit "solid black background" phrase into 13 still-failing prompts had a whitespace bug (`"([,}])` needed to be `"\s*([,}])` to match a prompt-only entry's `" }` closing, versus a two-field entry's `",` ) that made it overshoot into neighboring `MONSTER_ART` entries. It silently corrupted three unrelated, already-accepted creatures' prompts (`badger`, `giant-maggot`, `pugwampi`) before the diff was reviewed and the damage caught — all three were reverted to their original text (their already-generated, already-good images were untouched) before the intended 13 were fixed correctly by hand. Worth remembering: a multi-entry regex edit across this file needs its own care same as any other change, and diffing the result before trusting it is not optional.

**Redo tally: 28 of 41 needed at least one redo, several needed three rounds** — matching batch 3's escalation pattern (strengthen shared constants → per-creature `avoid` → explicit black-background phrase in the prompt itself → raised CFG → full prompt rewrite dropping the trigger word/phrase). `drover` (a herding-staff pose read as "standing in an open pasture" and produced a full desert/mountain/moon landscape) and `fisher` (a fishing-net pose read toward open water and produced a black-to-sand gradient) both needed their prompts rewritten from scratch, framed explicitly as a "centered studio portrait" up front rather than trusting the trailing background clause.
...
#### Reopened (2026-09-19): party turns were being auto-played too

**The user reports combat automated the party's turns as well as the NPCs'** — not what ITEM-8 was ever meant to do ("A real party member's own turn is never touched").

**Root cause, confirmed live via `foundry-rest` against the real deployed world.** The original guard (`combatant.actor?.hasPlayerOwner`) was verified only against a scratch actor with an explicit non-GM owner assigned — it was never checked against the real world's actual ownership setup. That real world has exactly **one user, a `Gamemaster`-role account, who owns all five party actors directly** (`ownership: { default: 1, "<gm-user-id>": 3 }`) — there is no separate player-role user at all. `Actor#hasPlayerOwner` only counts non-GM users with Owner permission, so it evaluates to `false` for every real party actor in this setup, identical to any NPC. Confirmed directly: reading `hasPlayerOwner` off all five live party actors returned `false` for each one, meaning the original guard let their turns auto-play right alongside real NPCs.

**Fix.** `dungeon-combat.mjs` no longer uses `hasPlayerOwner` as the sole signal. A new `partyActorIds()` helper (the same `game.actors.party.members` id-set `combatantTokens` already built inline) is checked first; `hasPlayerOwner` stays as a secondary OR-condition so a differently-configured world with real per-player accounts still works too. This matches how this file already defines "party" everywhere else, rather than depending on Foundry ownership permissions the world may not have set up the way ITEM-8's original scratch test assumed.

**Verification.** `npm test` — 627 passing, unchanged (no pure-function surface for this file, same precedent as the rest of it). Live-verified directly against the real world's actual data (not a scratch replica this time, since the bug was in a world-configuration assumption, not in reproducible pure logic): computed both the old and new guard conditions for all five real party actors and one real NPC actor (`Fey Dragonet`) already present in the world. Old guard: `hasPlayerOwner` false for every party actor → none would have been skipped (reproducing the bug exactly). New guard: every party actor now evaluates to skip-auto-play `true`; the NPC actor still evaluates to skip-auto-play `false` (still auto-plays, as intended). No live documents were created, mutated, or needed cleanup for this check.
...
#### Reopened (2026-09-20): a combat first room shouldn't prompt the GM at Start

**The user reports being shown the "Generate Encounter" Accept/Reroll preview immediately on clicking "Start Dungeon,"** stacked on top of the still-open Dungeon Crawl form itself (screenshot `Screenshot 2026-09-19 182637.png`) — before the party has even reached the dungeon. The user was clear the entry room itself is fine (it's genuinely always safe) — the objection is to the *first real room* (physical slot 1) getting built, populated, and prompted-for immediately at Start just because this item's own "exit always unlocked" design required it to already exist. Confirmed with the user directly: room 1 can still legitimately be a combat room (no change to generation odds), but its build/populate should wait for a GM action rather than happen automatically at Start — meaning its door needs to start locked again for that one case, contrary to this item's original "always unlocked" goal.

**Fix.** `dungeon-app.mjs`'s `#onStart` now only auto-builds/populates/unlocks the first real room immediately when it **isn't** combat (identical behavior to before for skill_challenge/puzzle_or_trap/narrative/safe_rest-as-room-1 — no change there, no prompt was ever involved). A combat first room's build is deferred entirely — its walls, its door, and its encounter don't exist yet right after Start. The already-existing "Populate Next Room" button/flow (`#onPopulateNext`, already shown whenever `nextRoomPending` is true — this item's own recovery path for a cancelled Accept/Reroll dialog on any room) now also builds the room's walls first via `buildRoomAtSlot` if they don't exist yet (a new `isSlotBuilt(scene, slot)` check in `dungeon-scene.mjs`, keyed off the connecting door's `dungeonDoorToSlot` flag) — a no-op for the normal case where the room was already built, so nothing about that flow changes for any other room. This reuses the item's own existing recovery UI/copy rather than adding a new button.

**A latent template bug this also fixed.** The tracker's `SafeEntryHint`/`SafeRestHint` ("the way onward is already open") were shown unconditionally whenever the current room was safe, with no check against `nextRoomPending` — already capable of showing directly alongside the *contradictory* "the way forward is locked" hint+button in the pre-existing cancelled-populate recovery case, just rarely reached before now. Wrapped both hints in `{{#unless nextRoomPending}}` in `dungeon-tracker.hbs` so only one message ever shows.

**Verification.** `npm test` — 669 passing, unchanged (no pure-function surface for this change; same precedent as the rest of `dungeon-scene.mjs`/`dungeon-app.mjs`). `npm run validate`/`validate:dungeon` unaffected. **Live verification was deliberately skipped** — a real, currently-active dungeon run was found on the live world (`Dungeon Crawl`, scene `jRp8fy1W7kAd6WJr`, presumably the user's own in-progress game where the bug was hit) at the time this fix was ready to test, and the normal live-test path (the real Start button, via `foundry-rest`) calls `scene.activate()` and `placePartyInSlot`'s `removeActorTokensFromAllScenes`, which would have forcibly evacuated the party from that real, active run. Shipped on code review instead: every Foundry-API-touching call reused here (`buildRoomAtSlot`, `populateSlotEncounter`, `unlockDoorToSlot`) is pre-existing and already live-verified (ITEM-9/10/11/21); the only new logic is `isSlotBuilt`, a trivial predicate against a wall flag shape (`dungeonDoorToSlot`) already directly observed multiple times this session. This change doesn't touch any hook or code path the user's existing in-progress run depends on (only `#onStart` and `#onPopulateNext`, both only run on an explicit future click), so it's safe to ship without disturbing that run — the user should verify the fix live the next time they click "Populate Next Room" or start a fresh dungeon.
