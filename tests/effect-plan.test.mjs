...
describe('planCardEffect — decides without writing', () => {
  it('reports the outcome while leaving the actor untouched', async () => {
    const actor = actorOf({ attributes: { speed: { value: 25 } } });
    const plan = await planCardEffect({
      card: cardOf('speed_bonus', { walk_ft: 10 }, 'Path'), actor
    });
    expect(plan.result.log).toBe('Path: walk speed 25 → 35 ft');
    expect(plan.calls).toHaveLength(1);
    // The plan is a description; the actor still reads 25.
    expect(actor.system.attributes.speed.value).toBe(25);
  });

  it('captures the write it intended to make', async () => {
    const plan = await planCardEffect({
      card: cardOf('speed_bonus', { walk_ft: 10 }), actor: actorOf({ attributes: { speed: { value: 25 } } })
    });
    expect(plan.calls[0]).toEqual({
      method: 'updateActor',
      args: ['a1', { 'system.attributes.speed.value': 35 }]
    });
  });
...
describe('replayPlan — the only thing that writes', () => {
  it('performs the planned calls in order', async () => {
    ...
    expect(api.done).toEqual([
      { method: 'updateActor', args: ['a1', { 'system.attributes.speed.value': 35 }] }
    ]);
  });
